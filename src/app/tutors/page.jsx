...
// // comend it
let allTutorsData = [];  // খালি array দিয়ে শুরু
try {
  const res = await fetch(url);
  if (!res.ok) {  // HTTP status 200-299 না হলে
    throw new Error(`HTTP error! status: ${res.status}`);
   }
  allTutorsData = await res.json();
...
console.log(allTutorsData, "Tutors");
    ...
    return (
        <div className='container mx-auto '> 
           ...
           {allTutorsData?.map(allTutor =>  < TutorCard key = {allTutor?._id} allTutor = {allTutor} />
